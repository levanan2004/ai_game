...
void main() {
  ...
  test('bouquet price sums sell prices and rounds to 1000', () {
    final b = bouquetOf({'rose': 3, 'baby': 2}, paper: 'kraft', ribbon: 'twine');
    final raw =
  ...
  test('tiers set pay factor and tip; fast bonus only for okay/great', () {
    const price = 40000;
    final great = computePayment(e, price: price, tier: Tier.great, fastService: false, wrapHit: false);
    expect(great.pay, price);
    expect(great.tip, roundTo1000(price * e.tiers['great']!.tipPercent));
    final fast = computePayment(e, price: price, tier: Tier.great, fastService: true, wrapHit: false);
    expect(
      fast.tip,
      roundTo1000(price * (e.tiers['great']!.tipPercent + e.fastServiceBonusPercent)),
    );
    final unhappy = computePayment(e, price: price, tier: Tier.unhappy, fastService: true, wrapHit: false);
  ...
  test('wrap hit adds max(minAmount, percent of price) on every tier', () {
    final small = computePayment(e, price: 10000, tier: Tier.unhappy, fastService: false, wrapHit: true);
    expect(small.wrapBonus, e.wrapBonusMin);
    final big = computePayment(e, price: 100000, tier: Tier.okay, fastService: false, wrapHit: true);
    expect(big.wrapBonus, max(e.wrapBonusMin, roundTo1000(100000 * e.wrapBonusPercent)));
    final miss = computePayment(e, price: 100000, tier: Tier.great, fastService: false, wrapHit: false);
  ...
  test('holiday and occasion tip multipliers apply', () {
    ...
    expect(p.tip, roundTo1000(100000 * e.tiers['great']!.tipPercent * 1.2 * 1.3));
  });
